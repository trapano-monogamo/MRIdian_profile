    ...
    def __init__(self, _scan_num, _raw_data, _begin, _end, _profile_out_dir, binning: float):
        ...
        for i in range(len(pos_data)):
            ...
            while t < 1.0:
                ...
                rebinned_first_derivative.append(utils.lerp(first_derivative[i], first_derivative[inext], t))
        ...
        first_derivative = [utils.gauss(x, *self.d1_left_fit_args) for x in rebinned_pos_data[:len(rebinned_pos_data) // 2]]
        first_derivative.extend([utils.gauss(x, *self.d1_right_fit_args) for x in rebinned_pos_data[len(rebinned_pos_data) // 2:]])
